...
var MochaTestRailReporter = (function (_super) {
    ...
    function MochaTestRailReporter(runner, options) {
        ...
        runner.on('pass', function (test) {
            ...
            if (caseIds.length > 0) {
                if (test.speed === 'fast') {
                    var testCases = caseIds.map(function (caseId) {
                        return {
                            caseId: caseId,
                            pass: true,
                            comment: test.title
                    ...
                    (_a = _this.testCases).push.apply(_a, testCases);
                }
                else {
                    var testCases = caseIds.map(function (caseId) {
                        return {
                    ...
                    (_b = _this.testCases).push.apply(_b, testCases);
                }
            }
            var _a, _b;
        });
        ...
        runner.on('end', function () {
            if (_this.testCases.length == 0) {
                console.warn("No testcases were matched. Ensure that your tests are declared correctly and matches TCxxx");
            }
            new testrail_1.TestRail(reporterOptions).publish(_this.passes, _this.fails, _this.pending, _this.out, _this.testCases);
        });
        return _this;
    }
